...
class MFRC522:
    ...
    def _tocard(self, cmd, send):
        ...
        if cmd == 0x0E:     # PCD_AUTHENT(0x0E)
            irq_en = 0x12
            wait_irq = 0x10
        elif cmd == 0x0C:   # PCD_TRANSCEIVE(0x0C)
            irq_en = 0x77
            wait_irq = 0x30
        ...
        self._wreg(0x01, 0x00)  # CommandReq(0x01), PCD_IDLE(0x00)

        for c in send:
            self._wreg(0x09, c) # Write_MFRC522(FIFODataReg(0x09), data))
        self._wreg(0x01, cmd)

        if cmd == 0x0C:     # PCD_TRANSCEIVE(0x0C)
            self._sflags(0x0D, 0x80)    #SetBitMask(BitFramingReg(0x0D), 0x80)

        i = 2000
        while True:
            n = self._rreg(0x04)    # Read_MRFC522(CommIrqReg(0x04))
            i -= 1
            if ~((i != 0) and ~(n & 0x01) and ~(n & wait_irq)):
                break

        self._cflags(0x0D, 0x80)    # ClearBitMask(BitFramingReg(0x0D), 0x80)

        if i:
            if (self._rreg(0x06) & 0x1B) == 0x00:   # ErrorReg(0x06)
                stat = self.OK

                if n & irq_en & 0x01:
                    stat = self.NOTAGERR
                elif cmd == 0x0C:       # PCD_TRANSCEIVE(0x0C)
                    n = self._rreg(0x0A)    # FIFOLevelReg(0x0A)
                    lbits = self._rreg(0x0C) & 0x07 # ControlReg(0x0C)
                    if lbits != 0:
                        bits = (n - 1) * 8 + lbits
                    else:
                        bits = n * 8
    ...
    def SelectTagSN(self):
        ...
        if self.DEBUG: print("Select Tag 1:",self.tohexstring(uid))
        if status != self.OK:
            return  (self.ERR,[])
    ...
    def getMifareUltraLight(self, page):
        return self.read(page * 4)
